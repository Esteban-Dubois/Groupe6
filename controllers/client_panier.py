...
# ajout dans le panier d'un article
    ...
    sql = "SELECT * FROM ligne_panier WHERE fusee_id_panier = %s AND id_utilisateur=%s"
    mycursor.execute(sql, (id_article, id_client))
    article_panier = mycursor.fetchone()
    
    mycursor.execute("SELECT * FROM fusee WHERE id_fusee = %s", (id_article))
    article = mycursor.fetchone()
    
    if not (article_panier is None) and article_panier['quantite'] >= 1:
        tuple_update = (quantite, id_client, id_article)
        sql = "UPDATE ligne_panier SET quantite = quantite+%s WHERE id_utilisateur = %s AND fusee_id_panier=%s"
        mycursor.execute(sql, tuple_update)
    else:
        tuple_insert = (id_client, id_article, quantite)
        sql = "INSERT INTO ligne_panier(id_utilisateur,fusee_id_panier,quantite, date_ajout) VALUES (%s,%s,%s, current_timestamp )"
