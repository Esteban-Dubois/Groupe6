...
def fct_fixtures_load():
    ...
    CREATE TABLE commande (
        ...
        adresse_id_facturer INT NOT NULL,
        adresse_id_livrer INT NOT NULL,
        PRIMARY KEY (id_commande),
        FOREIGN KEY (etat_id) REFERENCES etat(id_etat),
        FOREIGN KEY (utilisateur_id_commande) REFERENCES utilisateur(id_utilisateur),
        FOREIGN KEY (adresse_id_facturer) REFERENCES adresse(id_adresse),
        FOREIGN KEY (adresse_id_livrer) REFERENCES adresse(id_adresse)
    ) DEFAULT CHARSET utf8;
    '''
    mycursor.execute(sql)
    ...
    sql = "INSERT INTO couleur (libelle_couleur) VALUES ('Vert'), ('Jaune'), ('Orange'), ('Rouge'), ('Rose'), ('Violet'), ('Bleu'), ('Cyan');"
    mycursor.execute(sql)

    sql = "INSERT INTO etat (libelle_etat) VALUES ('En cours de traitement'), ('Expédié');"
    mycursor.execute(sql)

    sql = "INSERT INTO categorie (libelle_categorie) VALUES ('Fusée'), ('Chandelle'), ('fumigène'), ('Compact'), ('feu artifice portable'), ('fontaine');"
    mycursor.execute(sql)

    sql = '''
    INSERT INTO utilisateur(login,email,password,role,nom_utilisateur,est_actif) VALUES
    ('admin','admin@admin.fr','pbkdf2:sha256:1000000$eQDrpqICHZ9eaRTn$446552ca50b5b3c248db2dde6deac950711c03c5d4863fe2bd9cef31d5f11988','ROLE_admin','admin',1),
    ...
    sql = '''
    NSERT INTO fusee (prix_fusee, nom_fusee, stock_fusee, description_fusee, hauteur_explosion, duree_explosion, calibre_fusee, distance_securite, niveau_sonore, image_fusee, certification, effet, pays, couleur_id, categorie_id) VALUES
    ...
    (2.50, 'Compact tempete australe', 1000, 'Trois tubes individuels pour des départs précis et des effets visuels nets.', 18, 60, 15, 8, 95, 'feu_artifice6.png', 'CE F2', 'Feu nourri', 'Chine', 3, 4),
    (59.90, 'Detonator', 20, 'Génère une épaisse fumée colorée pendant une minute complète, idéal pour les photos ou événements.', 40, 30, 30, 25, 120, 'feu_artifice7.png', 'CE F3', 'Effets Brocarts', 'Chine', 6, 4),
    (14.00, 'Asian human', 120, 'Génère une épaisse fumée colorée pendant une minute complète, idéal pour les photos ou événements.', 25, 25, 20, 8, 105, 'feu_artifice8.png', 'CE F2', 'Pivoines classiques', 'Chine', 1, 4),
    (3.00, 'Giant maniax', 300, 'Un compact multi-effets haut de gamme pour un tableau complet et varié.', 20, 70, 15, 8, 100, 'feu_artifice9.png', 'CE F2', 'Rafales rapides', 'Chine', 4, 4),
    (22.50, 'Showbox fascination 5mn', 60, "Une batterie d'artifices nerveuse offrant une succession de gerbes lumineuses magiques.", 30, 300, 20, 25, 110, 'feu_artifice10.png', 'CE F3', 'Spectacle complet', 'Chine', 2, 5),
    (19.95, 'Vésuve magnum Argent ardi', 100, 'Un mini-compact ultra-abordable pour ajouter une touche de dynamisme à petit prix.', 5, 45, Null, 8, 80, 'feu_artifice11.png', 'CE F2', 'Fontaine conique', 'Chine', 8, 6),
    (1.75, 'Fontaine viper', 40, 'Un spectacle puissant et bruyant conçu pour marquer les esprits avec des détonations fortes.', 3, 30, Null, 8, 85, 'feu_artifice12.png', 'CE F2', 'Fontaine tubulaire', 'France', 7, 6),
    (4.50, 'Fusée Sifflet Infernal', 150, 'Un compact thématique offrant des effets visuels inspirés des traditions pyrotechniques asiatiques.', 30, 0, 15, 25, 110, 'feu_artifice13.png', 'CE F2', 'Bouquets aériens', 'Chine', 3, 1),
    (95.00, 'Panaché de 24 fusées de 50 mm', 10, "Un petit article de divertissement compact pour s'amuser en toute simplicité.", 50, 0, 50, 25, 120, 'feu_artifice14.png', 'CE F3', 'Effets variés', 'Chine', 5, 1),
    ...
    sql = '''
    INSERT INTO commande (date_achat, etat_id, utilisateur_id_commande, adresse_id_facturer, adresse_id_livrer) VALUES
    ('2025-04-08', 2, 2, 1, 1),
    ('2026-01-25', 1, 3, 2, 2),
    ('2025-12-20', 2, 2, 1, 1);
    '''
