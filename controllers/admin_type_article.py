...
def show_type_article():
    ...
    sql = ''' 
    SELECT 
    categorie.id_categorie AS id_type_article,
    categorie.libelle_categorie AS libelle,
    COUNT(fusee.id_fusee) AS nbr_articles
    FROM categorie
    LEFT JOIN fusee ON categorie.id_categorie = fusee.categorie_id
...
def valid_add_type_article():
    ...
    mycursor = get_db().cursor()
    sql = ''' 
    INSERT INTO categorie (libelle_categorie) VALUES (%s);
    '''
